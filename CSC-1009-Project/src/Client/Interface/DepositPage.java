...
public class DepositPage extends HomePage{
    public static void depositPage() throws IOException, InterruptedException {
        ConsoleClear.clear();
        ...
        while(!selection) { //while loop in case of wrong choices and switch for choices
            try {
                ...
                if (inputAmount.equalsIgnoreCase("back")) {
                    ...
                    if (amount <= 0) {
                        throw new IllegalArgumentException("You have to deposit more than 1" + currencyType);
                    } else {
                        System.out.print("Add a Memo (Press Enter If Non): ");
                        String memo = sc.nextLine();
                        account.depositOrWithdraw(amount, memo, true);
                        selection = true;
            ...
            } catch (IllegalArgumentException | InputMismatchException | IOException | InterruptedException e) {
